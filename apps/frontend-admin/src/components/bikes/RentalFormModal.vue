...
function submit() {
  ...
  if (!isTransportPaymentType(paymentType.value)) {
    ...
    void nextTick(() => document.getElementById('rental-paymentType')?.focus())
    return
  }
  emit('create', { courierId: courierId.value, paymentType: paymentType.value, startedAt: start, endedAt: end })
}
</script>
...
<template>
  <AppModal label-id="rentalFormTitle" modal-class="modal-confirm" data-od-id="rental-form-dialog" @close="$emit('close')">
    ...
    <form novalidate @submit.prevent="submit"><div class="modal-body">
      <div v-if="!rental" class="form-grid form-grid-single">
        <div class="field"><label class="required">Курьер</label><CourierPicker v-model="courierId" /></div>
        <div class="field"><label class="required" for="rental-startedAt">Начало аренды</label><input id="rental-startedAt" v-model="startedAt" class="input num" type="datetime-local" /></div>
        <div class="field"><label :class="{ required: requireHistorical }" for="rental-historicalEnd">Завершение{{ requireHistorical ? '' : ', если это история' }}</label><input id="rental-historicalEnd" v-model="historicalEnd" class="input num" type="datetime-local" /></div>
        <div class="field">
          <label class="required" for="rental-paymentType">Тип оплаты</label>
          <select id="rental-paymentType" v-model="paymentType" class="select" :disabled="saving" aria-describedby="rental-paymentType-hint">
          ...
          <span id="rental-paymentType-hint" class="admin-state-note">«Неделя назад» — оплата за прошедшую неделю.</span>
        </div>
      </div>
