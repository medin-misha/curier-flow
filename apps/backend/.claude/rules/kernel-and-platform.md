...
Общий стиль кода задан пользовательским правилом `code-style`. Сверх него в
`kernel` и `platform` действует следующее — потому что это код, который читают
и переиспользуют все модули, и ошибка в нём тиражируется:
