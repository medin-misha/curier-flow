...
- На первом этапе события не использовались; синхронизация контактов описана
  ниже в разделе «Последний арендатор».
...
Ответы списка и карточки транспорта содержат `last_rental`: `null`, если
истории нет, иначе краткую аренду с `id`, `started_at`, `ended_at`, `is_active`
и контактом `courier: {id, full_name, phone}`. После возврата последняя аренда
остаётся в ответе. Выбор идёт по `(started_at DESC, id DESC)`, поэтому внесение
прошлого периода задним числом не меняет последнего арендатора. При действующих
ограничениях непересечения активная аренда всегда начинается последней.

Список транспорта и фильтр `courier_id` сохраняют прежнюю семантику:
пагинация по `(created_at DESC, id DESC)`, фильтр только по активной аренде.
Контакты и последние аренды выбираются одним дополнительным запросом на
страницу, без полной загрузки истории, документов курьера или договоров.

Имя и фамилия хранятся существующим полем `full_name`. Контакты — актуальные
данные профиля, а не исторический снимок на момент выдачи. Модуль курьеров
публикует `courier.profile_changed` при создании и изменении имени/телефона,
`courier.deleted` при удалении; каждое событие атомарно записывается в outbox.
Подписчик транспорта обновляет `transport_courier_profiles`, не импортируя
соседний бизнес-модуль. Повторы и старые события не затирают новые контакты.
Удаление очищает имя и телефон и оставляет отметку, блокирующую восстановление
контакта запоздавшими событиями. Существующие правила каскадного удаления
неподписанной истории и защиты подписанных договоров сохраняются.

Доставка асинхронная: до обработки первого события `courier.id` и период
доступны, а `full_name` и `phone` равны `null`. Изменения профиля появляются
после обработки worker. Миграция заполняет контакты существующих курьеров и
добавляет индекс `(transport_id, started_at DESC, id DESC)`.
